...
if hasattr(torch.cuda, 'memory_stats'):
    print(
        f"初始 CUDA 内存状态: {torch.cuda.memory_allocated() / 1024**2:.2f} MB 已分配")
...
class DynamicActivationTrainer:
    ...
    def __init__(self, mypara):
        ...
        self.mymodel = create_supervised_geo_vmrnn(
            mypara,
            activation_config=self.activation_configs['physical_gating']
        ).to(mypara.device)

        # 添加模型參數統計
        total_params = sum(p.numel() for p in self.mymodel.parameters())
        trainable_params = sum(p.numel()
                               for p in self.mymodel.parameters() if p.requires_grad)
    ...
    def calscore(self, y_pred, y_true):
        ...
        with torch.no_grad():
            ...
            cor = (pred * true).sum(dim=0) / (
                torch.sqrt(torch.sum(pred ** 2, dim=0)
                           * torch.sum(true ** 2, dim=0))
                + 1e-6
            )
    ...
    def loss_var(self, y_pred, y_true, residual_losses=None, alpha=1.0):
        ...
        if residual_losses is not None:
            ...
            if isinstance(residual_losses, torch.Tensor):
                ...
                residual_term = alpha * \
                    torch.tensor(residual_losses, device=rmse.device)
        else:
    ...
    def train_model(self, dataset_train, dataset_eval, num_epochs=50):
        """訓練模型"""
        # 設置模型保存路徑
        model_name = "GeoVMRNN_resnet_DynamicActivation.pkl"
        chk_path = self.mypara.model_savepath + model_name
        torch.manual_seed(self.mypara.seeds)

        # 創建數據加載器 - 减小批次大小以降低内存使用
        batch_size_train = max(
            1, self.mypara.batch_size_train // 2)  # 减小训练批次大小
        batch_size_eval = max(
            1, self.mypara.batch_size_eval // 2)    # 减小评估批次大小

        print(
            f"原始训练批次大小: {self.mypara.batch_size_train}, 调整后: {batch_size_train}")
        print(
            f"原始评估批次大小: {self.mypara.batch_size_eval}, 调整后: {batch_size_eval}")
        ...
        mlflow.set_experiment("GeoVMRNN_DynamicActivation_latent_based")

        with mlflow.start_run():
            mlflow.set_tag("model", "GeoVMRNN_DynamicActivation_latent_based")
            mlflow.log_params({
            ...
            for i_epoch in range(num_epochs):
                ...
                for j, (input_var, var_true) in enumerate(dataloader_train):
                    ...
                    loss_var = self.loss_var(
                        var_pred, var_true.float().to(self.device), residual_loss)
                    ...
                    if hasattr(self.mymodel, 'fusion_activation') and hasattr(self.mymodel.fusion_activation, 'weights_history'):
                        if len(self.mymodel.fusion_activation.weights_history) > 0:
                            activation_weights = self.mymodel.fusion_activation.weights_history[-1]

                    if activation_weights is not None:
                        # 記錄激活函數權重
                        epoch_activation_weights.append(
                            activation_weights.detach().cpu())

                    # 使用普通損失（不再依賴 climate_labels）
                    ...
                    mlflow.log_metric("Train/Loss_Var",
                                      loss_var.item(), step=global_step)
                    mlflow.log_metric("Train/Loss_Nino",
                                      loss_nino.item(), step=global_step)
                    mlflow.log_metric("Train/Combine_Loss",
                                      combine_loss.item(), step=global_step)
                    mlflow.log_metric("Train/Score", score, step=global_step)
                    mlflow.log_metric("Train/tf_ratio",
                                      current_tf_ratio, step=global_step)
                    mlflow.log_metric("Train/Loss_Residual",
                                      residual_loss.item(), step=global_step)

                    # 記錄激活函數權重
                    if activation_weights is not None:
                        # activation_weights 可能是 [batch_size, num_activations] 或 [num_activations]
                        if activation_weights.dim() > 1:
                            # 如果是批次数据，先取平均
                            avg_weights = activation_weights.mean(dim=0)
                        else:
                            avg_weights = activation_weights
                        
                        # 逐个记录每个激活函数的权重
                        for i in range(avg_weights.shape[0]):
                            try:
                                weight_value = avg_weights[i].item() if torch.is_tensor(avg_weights[i]) else float(avg_weights[i])
                                mlflow.log_metric(f"Train/Activation_Weight_{i}", weight_value, step=global_step)
                            except Exception as e:
                                print(f"Warning: 無法记录激活权重 {i}: {e}")
                    global_step += 1
                    ...
                    if j % 100 == 0:
                        ...
                        if activation_weights is not None:
                            print(
                                f"Activation weights: {activation_weights.detach().cpu().numpy()}")

                        # 打印内存使用情况
                        if torch.cuda.is_available():
                            print(
                                f"CUDA 内存: {torch.cuda.memory_allocated() / 1024**2:.2f} MB 已分配, {torch.cuda.memory_reserved() / 1024**2:.2f} MB 已保留")
                    ...
                    if (i_epoch + 1 >= 4) and (j + 1) % 400 == 0:  # 从200改为400
                        ...
                        mlflow.log_metric(
                            "Eval/Loss_Var", eval_results['loss_var'], step=global_step)
                        mlflow.log_metric(
                            "Eval/Loss_Nino", eval_results['loss_nino'], step=global_step)
                        mlflow.log_metric(
                            "Eval/Combine_Loss", eval_results['combine_loss'], step=global_step)
                        mlflow.log_metric(
                            "Eval/Score", eval_results['score'], step=global_step)
                        ...
                        if 'activation_weights' in eval_results and eval_results['activation_weights'] is not None:
                            ...
                            for i in range(len(weights_array)):
                                try:
                                    weight_value = float(weights_array[i])
                                    mlflow.log_metric(f"Eval/Activation_Weight_{i}", weight_value, step=global_step)
                                except Exception as e:
                                    print(f"Warning: 無法记录评估权重 {i}: {e}")
                ...
                if epoch_activation_weights:
                    epoch_weights = torch.stack(
                        epoch_activation_weights).mean(dim=0)
                ...
                mlflow.log_metric("Epoch/Loss_Var",
                                  eval_results['loss_var'], step=i_epoch)
                mlflow.log_metric("Epoch/Loss_Nino",
                                  eval_results['loss_nino'], step=i_epoch)
                mlflow.log_metric("Epoch/Combine_Loss",
                                  eval_results['combine_loss'], step=i_epoch)
                mlflow.log_metric(
                    "Epoch/Score", eval_results['score'], step=i_epoch)

                # 記錄激活函數權重
                if 'activation_weights' in eval_results and eval_results['activation_weights'] is not None:
                    weights_array = eval_results['activation_weights']  # numpy array: [num_activations]
                    for i in range(len(weights_array)):
                        # 确保转换为 Python 标量
                        weight_value = float(weights_array[i])
                        mlflow.log_metric(f"Epoch/Activation_Weight_{i}", weight_value, step=i_epoch)
    ...
    def plot_activation_weights(self, epoch, is_final=False):
        ...
        for i in range(weights.shape[1]):
            plt.plot(weights[:, i], label=activation_names[i] if i < len(
                activation_names) else f"Activation {i}")
        ...
        if is_final:
            ...
            plt.savefig(os.path.join(
                save_dir, f'activation_weights_epoch_{epoch}.png'))
